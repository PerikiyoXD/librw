...
#ifdef RW_GL3
/* glad only. No <SDL.h>, no <GLFW/glfw3.h>: librw does not create windows or
 * contexts, so its public headers must not drag the host's windowing library
 * into every consumer. */
#include "glad/glad.h"
#endif
 ...
 * independently in a config header. gl3 is opened unconditionally above while
 * its contents are guarded, so an alias computed elsewhere could name a
