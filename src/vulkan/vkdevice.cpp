...
namespace rw
{
    ...
    namespace vulkan
    {
        ...
        static maple::TextureFilter filterConvMap_NoMIP[] = {
            maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear
        };

        static maple::TextureFilter filterConvMap_MIP[] = { maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear, maple::TextureFilter::Nearest, maple::TextureFilter::Linear };

        static maple::TextureWrap addressConvMap[] = { maple::TextureWrap::ClampToEdge, maple::TextureWrap::Repeat, maple::TextureWrap::MirroredRepeat, maple::TextureWrap::ClampToEdge, maple::TextureWrap::ClampToBorder };
...
#define MAXNUMSTAGES 8
            ...
            if (rwStateCache.texstage[stage].filter != (Texture::FilterMode)filter)
            {
                ...
                if (raster)
                {
                    ...
                    if (natras->filterMode != filter)
                    {
                        ...
                        getTexture(natras->textureId)
                            ->setSampler(maple::Sampler::create(
                                filterConvMap_MIP[filter], addressConvMap[rwStateCache.texstage[stage].addressingU], addressConvMap[rwStateCache.texstage[stage].addressingV], natras->maxAnisotropy, natras->numLevels));
                    ...
                    if (natras->maxAnisotropy != maxAniso)
                    {
                        getTexture(natras->textureId)
                            ->setSampler(maple::Sampler::create(
                                filterConvMap_MIP[filter], addressConvMap[rwStateCache.texstage[stage].addressingU], addressConvMap[rwStateCache.texstage[stage].addressingV], maxAniso, natras->numLevels));
        ...
        void setTextureBlend(Texture* tex)
        {
            ...
            else
            {
                ...
                if ((rw::bool32)natras->hasAlpha != rwStateCache.textureAlpha)
                {
                    rwStateCache.textureAlpha = (rw::bool32)natras->hasAlpha;
                    if (!rwStateCache.vertexAlpha)
                    {
        ...
        void setTexture(std::shared_ptr<maple::DescriptorSet> sets, int32 stage, Texture* tex)
        {
            ...
            else
            {
                ...
                getTexture(natras->textureId)
                    ->setSampler(maple::Sampler::create(
                        filterConvMap_MIP[tex->getFilter()], addressConvMap[rwStateCache.texstage[stage].addressingU], addressConvMap[rwStateCache.texstage[stage].addressingV], natras->maxAnisotropy, natras->numLevels));
        ...
        static void showRaster(Raster* raster, uint32 flags)
        {
            ...
            raytracingPrepare(vertexBuffers, indexBuffers, ssboLighting[bufferIdx], topLevel, getTextures(), skyBox, cmd);
            ...
            if (vertexBuffers.size() > 0)
            {
                ...
                if (EnablePathTrace.var)
                {
                    pathTrace = maple::path_trace::render(
                        ...
                        cmd,
                        deltaCameraPos.x > 0 || deltaCameraPos.y > 0 || deltaCameraPos.z > 0);
                }
                else
                {
                    ...
                    shadow = maple::shadow::render(
                        vertexBuffers, indexBuffers, ssboLighting[bufferIdx], topLevel, vkGlobals.currentDepth, getTextures(), reinterpret_cast<maple::vec3&>(cameraPos), reinterpret_cast<maple::mat4&>(viewProjInv), farPlane, nearPlane, cmd);
        ...
        static bool32 rasterRenderFast(Raster* raster, int32 x, int32 y)
        {
            ...
            case Raster::CAMERATEXTURE:
                ...
                case Raster::CAMERA:
					...
					auto from = std::static_pointer_cast<maple::Texture2D>(texture);
					auto to = std::static_pointer_cast<maple::Texture2D>(getTexture(natdst->textureId));
					maple::Texture2D::copy(from, to, cmdBuffer, x, (dst->height - src->height) - y, 0, 0, src->width, src->height);
                    return 1;
                }
                break;
            }
        ...
        static void makeVideoModeList(void)
        {
            ...
            vkGlobals.modes = rwNewT(DisplayMode, num + 1, ID_DRIVER | static_cast<int>(MEMDUR_EVENT));
