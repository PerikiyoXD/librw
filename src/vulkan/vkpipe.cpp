        ...
        void defaultInstanceCB(Geometry* geo, InstanceDataHeader* header, bool32 reinstance)
        {
            ...
            if (!reinstance)
            {
                ...
                if (isPrelit)
                {
                    ...
                    a->size   = 4;
                    a->type   = VK_FORMAT_R32G32B32A32_SFLOAT;
                    a->offset = stride;
                    stride += 16;
                    a++;
                }
                ...
                for (int32 n = 0; n < geo->numTexCoordSets; n++)
                {
                    ...
                    a->size   = 2;
                    a->type   = VK_FORMAT_R32G32_SFLOAT;
                    a->offset = stride;
                    stride += 8;
                    a++;
                }
                ...
                header->attribDesc = rwNewT(AttribDesc, header->numAttribs, static_cast<int>(MEMDUR_EVENT) | static_cast<int>(ID_GEOMETRY));
            ...
            if (!reinstance || geo->lockedSinceInst & Geometry::LOCKVERTICES)
            {
                ...
                instV3d(VERT_FLOAT3, verts + a->offset, geo->morphTargets[0].vertices, header->totalNumVertex, a->stride);
            }
            ...
            for (int32 n = 0; n < geo->numTexCoordSets; n++)
            {
                if (!reinstance || geo->lockedSinceInst & (Geometry::LOCKTEXCOORDS << n))
                {
                    ...
                    instTexCoords(VERT_FLOAT2, verts + a->offset, geo->texCoords[n], header->totalNumVertex, a->stride);
                }
            }
